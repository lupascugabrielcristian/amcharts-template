...
import { HistoricalDataItem, HistoricalDataResponse } from 'src/models/stockDataSrc';
import { StockDataAmChartItem } from 'src/models/stockData';

@Component({
  selector: 'app-test-stock-chart',
  templateUrl: './test-stock-chart.component.html',
  styleUrls: ['./test-stock-chart.component.scss']
})
export class TestStockChartComponent implements OnInit {
  ...
  ngAfterViewInit() {
    ...
    // Create value axis and date axis
    ...
    let historicalDataUrl = "https://api.stockdata.org/v1/data/eod?symbols=AAPL&api_token=HY3uOw9uQwBfDoKA2J2lg5ifK4xrFzPV9W2ZM3wG";
    const historicalDataObs = this.http.get<HistoricalDataResponse>(historicalDataUrl);

    historicalDataObs.subscribe( (historicalData: HistoricalDataResponse) => {
      console.log("Returned %o", historicalData );

      const newData: StockDataAmChartItem[] = historicalData.data.slice(0,50).map( this.convertDataItem );
  ...
  private convertDataItem(item: HistoricalDataItem): StockDataAmChartItem {
    const d = new Date(item.date);

    let res = new StockDataAmChartItem();
    res.Date = +d;
    res.Open = item.open;
    res.High = item.high;
